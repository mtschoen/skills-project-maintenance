...
Instead, before running the rest of the checklist, PM re-runs the read-only detection side of wrap's per-session hygiene items itself, scoped to this repo only. The canonical item list and its detection guidance live in wrap's `references/hygiene-checklist.md` - read that file rather than duplicating its table here; it is the single source of truth for the per-session items, and this section deliberately does not enumerate them so it cannot drift stale. Run each item's "What to look for" detection query from that table (all read-only - `git status --porcelain`, `git log @{u}..HEAD`, and so on) against this repo, and turn every hit into a normal PM finding per `references/finding-schema.md`: evidence, recommendation, confidence, and the exact action on approval. Feed those findings into step 2's research pass alongside everything else PM finds.
