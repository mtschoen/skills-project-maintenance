...
### 0. Hygiene survey (findings-only, PM-owned)

PM is a single-repo, audit-tier pass. It does not invoke the `wrap` skill: wrap has no non-interactive "findings-only" variant - even `wrap --fast` still executes writes autonomously - and its full run is its own five-phase, whole-session ritual (multi-repo scope detection across every repo the session touched, background-process and teammate-agent teardown, a mandatory closing sentinel telling the user the session is safe to close). Invoking it from inside PM, especially from a fleet subagent, would sweep repos PM was never asked to touch, tear down live background work, and emit a session-close instruction nobody asked for. That ritual stays wrap's own thing, invoked separately as `/wrap` when the user wants it.

Instead, before running the rest of the checklist, PM re-runs the read-only detection side of wrap's per-session hygiene items itself, scoped to this repo only. The canonical item list and its detection guidance lives in wrap's `references/hygiene-checklist.md` - read that file rather than duplicating its table here. At the time of writing it covers: uncommitted changes, unpushed commits, tracked and untracked temp files, project-scoped stale memory, merged local branches, stale worktrees the current session created, `.claude/scripts/` one-offs, junk/build-artifact dirs the session generated, and docs drift. Run each item's "What to look for" detection query from that table (all read-only - `git status --porcelain`, `git log @{u}..HEAD`, and so on) against this repo, and turn every hit into a normal PM finding per `references/finding-schema.md`: evidence, recommendation, confidence, and the exact action on approval. Feed those findings into step 2's research pass alongside everything else PM finds.

**Never execute a fix in this step.** Detection only - nothing here writes to disk or runs a mutating git command, even for items wrap itself would auto-fix or auto-delete. Every hit from this survey rides PM's standard propose/approve flow (steps 2-3), governed by PM's own stricter operating principles (see principle 1 above).

In fleet-subagent mode, this survey is findings-only automatically: it is just PM's own fleet contract (see "Fleet subagent" above) applied to this step like every other step. There is nothing to inherit from wrap here, because PM never runs wrap's code.
