...
**Relationship to the `wrap` skill:** wrap and project-maintenance split hygiene by scope and by mode, not by delegation. project-maintenance never invokes wrap - wrap has no findings-only mode, and its full run is a five-phase, whole-session ritual (multi-repo scope detection across every repo the session touched, background-process and teammate-agent teardown, a mandatory closing sentinel) that has no business running as a step inside a single-repo maintenance pass. Per `wrap`'s `references/hygiene-checklist.md` (shipped with the wrap skill; look under the installed wrap skill's directory, not this repo), wrap owns the *interactive, autonomous-fix* version of the per-session hygiene items (dirty trees, temp files, stale memory, merged branches, stale worktrees); project-maintenance's step 0 (below) re-runs the *findings-only, detection* version of that same item list directly against this one repo, feeding any hits into project-maintenance's own propose/approve flow instead of executing them. project-maintenance's own checklist below covers the rare/audit-tier items that sit entirely outside that list (default-branch renames, the agent-instruction file convention - AGENTS.md source of truth with `@AGENTS.md` import pointers, missing README/LICENSE/.gitignore, dead code, large tracked files, disk warnings, and cross-project config drift - on-save linter hook, CI, aislop gate). When a session is ending and a maintenance pass was requested, run project-maintenance on the target repo and let `/wrap` handle session-wide concerns separately - not the reverse. If you are running in an interactive session and the user actually wants a full end-of-session hygiene pass across everything the session touched, prefer `/wrap` directly.

## Operating principles

1. **Verify before delete.** Never delete untracked files. For tracked files, delete only when (a) the working tree is clean and (b) you can state why the file has served its purpose. This applies uniformly, including to untracked-file findings that step 0's hygiene survey turns up - wrap's own operating rules allow deleting untracked files with per-item approval, a looser rule that fits its scratch-cleanup purpose, but project-maintenance never delegates execution to wrap, so project-maintenance's stricter rule is what governs every finding this skill produces, step 0 included.
...
### 0. Hygiene survey (findings-only, project-maintenance-owned)

project-maintenance is a single-repo, audit-tier pass. It does not invoke the `wrap` skill: wrap has no non-interactive "findings-only" variant - even `wrap --fast` still executes writes autonomously - and its full run is its own five-phase, whole-session ritual (multi-repo scope detection across every repo the session touched, background-process and teammate-agent teardown, a mandatory closing sentinel telling the user the session is safe to close). Invoking it from inside project-maintenance, especially from a fleet subagent, would sweep repos project-maintenance was never asked to touch, tear down live background work, and emit a session-close instruction nobody asked for. That ritual stays wrap's own thing, invoked separately as `/wrap` when the user wants it.

Instead, before running the rest of the checklist, project-maintenance re-runs the read-only detection side of wrap's per-session hygiene items itself, scoped to this repo only. The canonical item list and its detection guidance live in wrap's `references/hygiene-checklist.md` (wrap skill's own directory) - read that file rather than duplicating its table here; it is the single source of truth for the per-session items, and this section deliberately does not enumerate them so it cannot drift stale. Run each item's "What to look for" detection query from that table (all read-only - `git status --porcelain`, `git log @{u}..HEAD`, and so on) against this repo, and turn every hit into a normal project-maintenance finding per `references/finding-schema.md`: evidence, recommendation, confidence, and the exact action on approval. Feed those findings into step 2's research pass alongside everything else project-maintenance finds.

**Never execute a fix in this step.** Detection only - nothing here writes to disk or runs a mutating git command, even for items wrap itself would auto-fix or auto-delete. Every hit from this survey rides project-maintenance's standard propose/approve flow (steps 2-3), governed by project-maintenance's own stricter operating principles (see principle 1 above).

In fleet-subagent mode, this survey is findings-only automatically: it is just project-maintenance's own fleet contract (see "Fleet subagent" above) applied to this step like every other step. There is nothing to inherit from wrap here, because project-maintenance never runs wrap's code.
