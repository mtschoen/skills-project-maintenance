...
`evals/` holds a runner/grader pair cloned from the docs-update harness, adapted for git-state scenarios: per-scenario `setup.sh` builds the planted repo state (branches, bare remotes, empty directories - state a checked-in seed cannot carry), the agent runs a maintenance pass via `claude -p`, and `probe.sh` captures post-run git state as key=value lines for grading. Outcome buckets: `actioned | surfaced | missed | over_actioned | no_op`. Five scenarios cover PM's checks: merged branch lingering on a remote, empty directory husks, and multi-remote mirror drift (the rare/audit-tier checks PM owns directly); memory update-in-place (step 0's shared hygiene survey, testing PM's own update-in-place research logic on a stale-memory hit); and a healthy-repo control where any "cleanup" is destructive.
